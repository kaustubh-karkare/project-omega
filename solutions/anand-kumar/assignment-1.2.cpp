...
void parameter_declaration(vector <struct parameter_node> &parameter, string
													 p_name, string p_shorthand, string p_data_type, 
													 string p_help) {
...
//  Extract the string content at any inder of argv
...
void parameter_argument(vector <struct parameter_node> parameter, int argc, 
	...
	if (parameter[option_index].parameter_data.first == false) {
		...
		if (valid_data_type == false) {
			printf("Argument is either invalid or not provided\n");
			exit(1);
		}
...
void help_function(vector <struct parameter_node> parameter) {
	for ( int i = 0; i < parameter.size(); ++i) {
		...
		parameter[i].parameter_shorthand<<" "<<parameter[i].parameter_help<<endl;
	}
}
...
int main(int argc, char *argv[]) {
	...
	/*	Enter the parameter name, shorthand representaion(optional), data type and
			...
			Function definition:- void parameter_declaration(
														vector <struct parameter_node> &parameter, string
													 	p_name, string p_shorthand, string p_data_type, 
													 	string p_help);
	...
	*/
	...
	/*	Enter the list of mutually exclusive elements in an array of string.
			...
			Function definition:- void create_exclusive_parameters(
														vector <struct parameter_node> parameter, 
														string ex_list[], int s);
	*/
	//	Test data 
	...
	int index = 1;
	if (argc < 2) {
		printf("No parameter or command given\n");
		exit(1);
	}
	int counter = 0;
	while(index < argc) {
		...
		else {	//	If a command or a sub command.
